...
class DataLoader():
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            masked_samples = []
            samples = []
            seq_len = random.randint(self.seq_len // 8, self.seq_len // 4 - 1) * 8
            for _ in range(self.batch_size):
                start = random.randint(0, len(self.tokenized_corpus) - seq_len - 1)
                tokens = self.tokenized_corpus[start: start + seq_len]
